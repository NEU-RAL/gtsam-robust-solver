...
/**
 * @file IRLSExample.cpp
 * @brief Simple example showcasing a Iteration Reweighted Least Squares based solver
...
#include <gtsam/nonlinear/IrlsOptimizer.h>
...
int main() {
  cout << "Iteration Reweighted Least Squares with Truncated Least Squares Example\n";
  ...
  // Set IRLS-specific options
  IrlsParams<LevenbergMarquardtParams> irlsParams(lmParams);
  irlsParams.setLossType(IrlsLossType::TLS);

  // Optimize the graph and print results
  IrlsOptimizer<IrlsParams<LevenbergMarquardtParams>> optimizerTLS(graph, initial, irlsParams);
  Values resultTLS = optimizerTLS.optimize();
  resultTLS.print("Final Result:");


  cout << "Iteration Reweighted Least Squares with Geman-McClure Example\n";

  // Set IRLS-specific options
  irlsParams.setLossType(IrlsLossType::GM);

  // Optimize the graph and print results
  IrlsOptimizer<IrlsParams<LevenbergMarquardtParams>> optimizerGM(graph, initial, irlsParams);
