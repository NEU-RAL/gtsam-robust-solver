...
/**
 * @file    FracgmOptimizer.h
 * @brief   The FracgmOptimizer class
 * @author  Bang-Shien Chen
 *
 * Implementation of the paper: 
 * FracGM: A Fast Fractional Programming Technique for Geman-McClure Robust Estimator
 * 
 * NOTE the weights are $\mu(c^2-\beta)$ by expanding the objective in Lemma 1
 * TODO add the norm of $\psi$ as a stopping criteria
 * TODO access information of $\mu$ and $\beta$
 */

#pragma once

#include <gtsam/nonlinear/FracgmParams.h>
...
/* ************************************************************************* */
template<class FracgmParameters>
class FracgmOptimizer {
 public:
  /// For each parameter, specify the corresponding optimizer: e.g., GaussNewtonParams -> GaussNewtonOptimizer.
  typedef typename FracgmParameters::OptimizerType BaseOptimizer;

 private:
  NonlinearFactorGraph nfg_; ///< Original factor graph to be solved by FracGM.
  Values state_; ///< Initial values to be used at each iteration by FracGM.
  FracgmParameters params_; ///< FracGM parameters.
  Vector weights_;  ///< Weights associated to each factor in FracGM (this could be a local variable in optimize, but it is useful to make it accessible from outside).
 ...
 public:
  /// Constructor.
  FracgmOptimizer(const NonlinearFactorGraph& graph, const Values& initialValues,
               const FracgmParameters& params = FracgmParameters())
    ...
    if(inconsistentlySpecifiedWeights.size() > 0){ // if we have inconsistently specified weights, we throw an exception
      params.print("params\n");
      throw std::runtime_error("FracgmOptimizer::constructor: the user has selected one or more measurements"
  ...
  void setWeights(const Vector w) {
    if (size_t(w.size()) != nfg_.size()) {
      throw std::runtime_error(
          "FracgmOptimizer::setWeights: the number of specified weights"
  ...
  const FracgmParameters& getParams() const { return params_;}

  /// Access a copy of the FracGM weights.
  ...
  /// Equals.
  bool equals(const FracgmOptimizer& other, double tol = 1e-9) const {
  ...
  /// Compute optimal solution using FracGM.
  Values optimize() {
    ...
    // ^^ number of measurements that are not known to be inliers or outliers (FracGM will need to figure them out)
    if (nrUnknownInOrOut == 0) { // no need to even call FracGM in this case
      if (nrUnknownInOrOut==0 && params_.verbosity >= FracgmParameters::Verbosity::SUMMARY) {
        std::cout << "FracGM Optimizer stopped because all measurements are already known to be inliers or outliers"
                  << std::endl;
      }
      if (params_.verbosity >= FracgmParameters::Verbosity::VALUES) {
        result.print("result\n");
      }
      return result;
    }
    ...
    for (iter = 0; iter < params_.maxIterations; iter++) {
      ...
      // display info
      if (params_.verbosity >= FracgmParameters::Verbosity::WEIGHTS) {
      ...
      if (checkConvergence(cost, prev_cost)) {
        break;
      }
    ...
    // display info
    if (params_.verbosity >= FracgmParameters::Verbosity::SUMMARY) {
  ...
  bool checkCostConvergence(const double cost, const double prev_cost) const {
    ...
    if (costConverged && params_.verbosity >= FracgmParameters::Verbosity::SUMMARY){
  ...
  /// Check for convergence between consecutive FracGM iterations.
  // TODO change (or add) stopping criteria by the norm of $\psi$ (should still work with relative cost)
  bool checkConvergence(const double cost, const double prev_cost) const {
    return checkCostConvergence(cost, prev_cost);
  }

  /// Create a graph where each factor is weighted by the FracGM weights.
    ...
    for (size_t i = 0; i < nfg_.size(); i++) {
      if (nfg_[i]) {
        ...
        if (noiseModel) {
          ...
          throw std::runtime_error(
              "FracgmOptimizer::makeWeightedGraph: unexpected non-Gaussian noise model.");
        }
      }
    }
    return newGraph;
  ...
  Vector calculateWeights(const Values& currentEstimate) {
    ...
    // NOTE the weights are $\mu(c^2-\beta)$ by expanding the objective in Lemma 1
    // since we are alternatively minimizing $x$ here, the constant term can be ignored
    std::vector<double> beta(unknownWeights.size()); 
    std::vector<double> mu(unknownWeights.size()); 

    for (size_t k : unknownWeights) {
      if (nfg_[k]) {
        double u2_k = nfg_[k]->error(currentEstimate);  // squared (and whitened) residual
        mu[k] = 1 / (u2_k + barcSq_[k]);  // Equation (7)
        beta[k] = (u2_k * barcSq_[k]) / (u2_k + barcSq_[k]);  // Equation (8)
        weights[k] = mu[k] * (barcSq_[k] - beta[k]);  // Lemma (1)
      }
    }
    return weights;
  }
