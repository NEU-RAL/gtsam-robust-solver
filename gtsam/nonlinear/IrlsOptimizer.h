...
/**
 * @file    IrlsOptimizer.h
 * @brief   The IrlsOptimizer class
 * @author  Bang-Shien Chen
 */

#pragma once

#include <gtsam/nonlinear/IrlsParams.h>
...
/* ************************************************************************* */
template<class IrlsParameters>
class IrlsOptimizer {
 public:
  /// For each parameter, specify the corresponding optimizer: e.g., GaussNewtonParams -> GaussNewtonOptimizer.
  typedef typename IrlsParameters::OptimizerType BaseOptimizer;

 private:
  NonlinearFactorGraph nfg_; ///< Original factor graph to be solved by IRLS.
  Values state_; ///< Initial values to be used at each iteration by IRLS.
  IrlsParameters params_; ///< IRLS parameters.
  Vector weights_;  ///< Weights associated to each factor in IRLS (this could be a local variable in optimize, but it is useful to make it accessible from outside).
 ...
 public:
  /// Constructor.
  IrlsOptimizer(const NonlinearFactorGraph& graph, const Values& initialValues,
               const IrlsParameters& params = IrlsParameters())
    ...
    if(inconsistentlySpecifiedWeights.size() > 0){ // if we have inconsistently specified weights, we throw an exception
      params.print("params\n");
      throw std::runtime_error("IrlsOptimizer::constructor: the user has selected one or more measurements"
  ...
  void setWeights(const Vector w) {
    if (size_t(w.size()) != nfg_.size()) {
      throw std::runtime_error(
          "IrlsOptimizer::setWeights: the number of specified weights"
  ...
  const IrlsParameters& getParams() const { return params_;}

  /// Access a copy of the IRLS weights.
  ...
  /// Equals.
  bool equals(const IrlsOptimizer& other, double tol = 1e-9) const {
  ...
  Values optimize() {
    ...
    // ^^ number of measurements that are not known to be inliers or outliers (IRLS will need to figure them out)
    if (nrUnknownInOrOut == 0) { // no need to even call IRLS in this case
      if (nrUnknownInOrOut==0 && params_.verbosity >= IrlsParameters::Verbosity::SUMMARY) {
        std::cout << "IRLS Optimizer stopped because all measurements are already known to be inliers or outliers"
                  << std::endl;
      }
      if (params_.verbosity >= IrlsParameters::Verbosity::VALUES) {
        result.print("result\n");
      }
      return result;
    }
    ...
    for (iter = 0; iter < params_.maxIterations; iter++) {
      ...
      // display info
      if (params_.verbosity >= IrlsParameters::Verbosity::WEIGHTS) {
    ...
    // display info
    if (params_.verbosity >= IrlsParameters::Verbosity::SUMMARY) {
  ...
  bool checkCostConvergence(const double cost, const double prev_cost) const {
    ...
    if (costConverged && params_.verbosity >= IrlsParameters::Verbosity::SUMMARY){
  ...
  /// Check for convergence between consecutive IRLS iterations.
  bool checkConvergence(const double cost, const double prev_cost) const {
    return checkCostConvergence(cost, prev_cost);
  }

  /// Create a graph where each factor is weighted by the IRLS weights.
    ...
    for (size_t i = 0; i < nfg_.size(); i++) {
      if (nfg_[i]) {
        ...
        if (noiseModel) {
          ...
          throw std::runtime_error(
              "IrlsOptimizer::makeWeightedGraph: unexpected non-Gaussian noise model.");
        }
      }
    }
    return newGraph;
  ...
  Vector calculateWeights(const Values& currentEstimate) {
    ...
    switch (params_.lossType) {
      case IrlsLossType::GM: { 
        for (size_t k : unknownWeights) {
          if (nfg_[k]) {
            double u2_k = nfg_[k]->error(currentEstimate);  // squared (and whitened) residual
            weights[k] = std::pow(
                (std::sqrt(2.0) * barcSq_[k]) / (u2_k + barcSq_[k]), 2);
          }
        }
        return weights;
      }
      case IrlsLossType::TLS: { 
        for (size_t k : unknownWeights) {
          if (nfg_[k]) {
            double u2_k = nfg_[k]->error(currentEstimate);  // squared (and whitened) residual
            if (u2_k >= barcSq_[k] || weights[k] < 0) {
              weights[k] = 0;
            } else if (u2_k <= barcSq_[k] || weights[k] > 1) {
              weights[k] = 1;
            }
          }
        }
      ...
      default:
        throw std::runtime_error(
            "IrlsOptimizer::calculateWeights: called with unknown loss type.");
    }
  }
};
